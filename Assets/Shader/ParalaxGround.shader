Shader "Volume/ParalaxGround"
{
	Properties
	{
		...
		_Height("Height", Range(0.0001,1)) = 1.0
		...
		SubShader
		{
			...
			float4 getNormalTexture(float2 texturePos, float height)
			{
				...
				float4 colorNormal = tex2Dlod(_NormalMap, float4(texturePos * textureSize, 0, 0));
			...
			void surf(Input IN, inout SurfaceOutputStandardSpecular o)
			{
				...
				for (int i = 0; i < STEPS; i++)
				{
					...
					if (rayPos.y < height)
					{
						...
						finalNormal = getNormalTexture(weightedTex, height);
				...
				//o.Albedo = finalNormal.rgba;
				o.Albedo = finalColor.rgba;
				o.Normal = UnpackNormal(finalNormal.rgba);
				//o.Gloss = tex2D(_GlossMap, IN.uv_GlossMap).a * _Gloss;
				o.Specular = tex2D(_GlossMap, IN.uv_GlossMap).a * _Gloss;
